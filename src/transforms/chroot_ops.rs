...
impl Transform for ChrootOps {
    ...
    fn check(&self, ctx: &super::Context) -> Result<bool> {
        ...
        let modules = dir
            .open_dir("usr/lib/modules")
            .context("opening usr/lib/modules")?;
        for entry in modules.entries().context("reading usr/lib/modules")? {
        ...
        if dir.exists("usr/bin/bootupctl") && !dir.exists("usr/lib/bootupd/updates") {
            return Ok(false);
        }
